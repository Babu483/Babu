...
def configure_ngrok():
    ngrok_token = "2pOtBBxsUjiJUldWiafxY0Yxi0v_6uuuokAiMpc2mK8XTnjSK"
    try:
...
def update_soul_txt(public_url):
    with open("babu.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in babu.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("babu.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated babu.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update babu.txt on VPS: {str(e)}")
